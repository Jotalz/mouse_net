...
class Game_settings:
    ...
    def randomize(self):
        if Option_random_screen_resolution:
            ...
            self.screen_width = random.randint(Option_random_screen_resolution_width[0],
                                               Option_random_screen_resolution_width[1])
            self.screen_height = random.randint(Option_random_screen_resolution_height[0],
                                                Option_random_screen_resolution_height[1])
        ...
        if Option_random_mouse_sensitivity:
            self.mouse_sensitivity = random.uniform(Option_random_mouse_sensitivity_min_max[0],
                                                    Option_random_mouse_sensitivity_min_max[1])
...
if __name__ == "__main__":
    delete_pre = input("是否删除之前的数据和模型(y/n)：")
    train = input("是否启用训练(y/n)：")
    generate = input("是否生成训练数据(y/n)：")
    vision = input("是否启用训练可视化(y/n)：")
    test = input("是否启用测试(y/n)：")
    ###################### Options ######################
    # Game settings
    ...
    # Data
    Option_delete_prev_data = False if delete_pre in ["0", "n", "no", "N", "NO"] else True
    ...
    # Testing model
    Option_test_model = False if test in ["0", "n", "no", "N", "NO"] else True
